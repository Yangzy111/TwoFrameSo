...
namespace Position
{
    ...
    public:
        ...
        //推算位姿
        virtual bool estimate(cv::Mat &R, cv::Mat &t, MatchVector &matches, Pt3Vector &vPts, cv::Mat &F)
        {
            assert(NULL);
            return false;
        }
        //计算基础矩阵
        virtual Mat calcFundamental(MatchVector &matches, U8Vector &status) 
        {
            assert(NULL);
            return Mat();
        }
    ...
    public:
        ...
        //推算位姿
        virtual bool estimate(cv::Mat &R, cv::Mat &t,MatchVector &matches, Pt3Vector &vPts, cv::Mat &FH);
