...
#pragma region ORBPoseEstimation
    ...
    //计算基础矩阵
    Mat ORBPoseEstimation::calcFundamental(MatchVector &matches, U8Vector &status)
    {
        assert(NULL);
    }
    ...
    //估计
    bool ORBPoseEstimation::estimate(cv::Mat &R, cv::Mat &t, MatchVector &matches, Pt3Vector &vPts, cv::Mat &FH)
    {
        assert(mPre && mCur);
        bool bol = false;
        const int maxInterator = 10;//最大迭代次数,暂时取其中成功的一组作为位姿条件(并不是最优)
        int interator = 0;
        while(!bol)
        {
...
#pragma region CVPoseEstimation
    ...
    //计算基础矩阵
    Mat CVPoseEstimation::calcFundamental(MatchVector &matches, U8Vector &status)
    {
        initParams(matches);
        if(mPrePts.empty() || (mPrePts.size() != mCurPts.size()))
            return Mat();
        cv::findFundamentalMat(mPrePts,mCurPts,status,CV_RANSAC,3);
    }

    //推算位姿
    bool CVPoseEstimation::estimate(cv::Mat &R, cv::Mat &t, MatchVector &matches, Pt3Vector &vPts, cv::Mat &F)
    {
        initParams(matches);
        if(mPrePts.empty() || (mPrePts.size() != mCurPts.size()))
            return false;
        ...
        F = PUtils::ComputeFundamentalMat(E, mCam.K, mCam.K);
