...
void FeatureQuadTree::GenerateFeatureQuadTree(const Mat &img, KeyPtVector inputkeypts, KeyPtVector &outputkeypts)
{   
    if(!mFeature)
        mFeature = cv::xfeatures2d::SIFT::create(2000,8,0.05,15,1.4);
    ...
    for(int i=0; i<nRows; i++)
    {
        ...
        for(int j=0; j<nCols; j++)
        {
            ...
            for(int k=0; k<inputkeypts.size(); k++)
            {
                if(inputkeypts[k].pt.x>iniX&&inputkeypts[k].pt.x<maxX&&inputkeypts[k].pt.y>iniY&&inputkeypts[k].pt.y<maxY)
                {
                    //cv::KeyPoint kpcell;
                    inputkeypts[k].pt.x = inputkeypts[k].pt.x-iniX;
                    inputkeypts[k].pt.y = inputkeypts[k].pt.y-iniY;
                    vKeysCell.emplace_back(inputkeypts[k]);
                }
                    
            }

            //cout<<"vKeysCell: "<<vKeysCell.size()<<endl;
            //mFeature->detect(img.rowRange(iniY,maxY).colRange(iniX,maxX),vKeysCell);
    ...
    //mMaxFeatures /= ((nRows - 1) * nCols);
    //cout << "key before filter:" << vToDistributeKeys.size() << endl;
    outputkeypts = distributeQuadTree(vToDistributeKeys, minBorderX, maxBorderX,
                                  minBorderY, maxBorderY,mMaxFeatures);

    //cout << "key after filter:" << outputkeypts.size() << endl;

    // Add border to coordinates and scale information
    const int nkps = outputkeypts.size();
    for(int i=0; i < nkps ; i++)
    {
        outputkeypts[i].pt.x+=minBorderX;
        outputkeypts[i].pt.y+=minBorderY;
    }
    //  Mat outimg = img.clone();
    // keypts.swap(keypoints);
    // static int index = 0;
