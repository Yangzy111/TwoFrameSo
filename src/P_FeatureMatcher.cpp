...
#define SEARCHLEVEL     0    //匹配搜索层级
    ...
    //匹配  返回匹配对
    MatchVector PFeatureMatcher::match(IFrame *preframe, IFrame *curframe, int windowsize, float ratio)
    {
    ...
    //匹配  返回匹配对
    MatchVector PKnnMatcher::match(IFrame *preframe, IFrame *curframe, int windowsize, float ratio)
    {
    assert(preframe);
        assert(curframe);
